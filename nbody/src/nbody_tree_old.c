...
/* Find k nearest neighbors (here k=4) for body p using the NBodyTree.
 * Results are written into out[], sorted by distance (closest first).
 * Returns the number of neighbors actually found (≤4).
 */
//#define K 4
#define KN 32


static void insertNeighbor(Body* neighbors[], real dists[], int K, Body* b, real dist2)
{
    //mw_printf("running insertNeighbor\n");
    if (b->bodynode.type>-1){
	//mw_printf("not dark matter?\n");
	return;
    }
    int worst = 0;
    real worstDist = dists[0];
    for (int i = 1; i < K; i++)
        if (dists[i] > worstDist) { worst = i; worstDist = dists[i]; }
    //if (dist2 < worstDist && dist2 < 0.91)
    if (dist2 < worstDist)
    {
        dists[worst] = dist2;
        neighbors[worst] = b;
...
static void searchNodeSafe(const NBodyTree* t, const NBodyNode* node, const Body* p,
                           Body* neighbors[], real dists[], int K)
{
    if (node == NULL) return;

    uintptr_t addr = (uintptr_t)node;
    if (addr < 0x1000 || addr > 0x7fffffffffff) {
        //mw_printf("Skipping invalid node pointer %p\n", node);
        return;
    }

    if (isBody(node))
    {
	mw_printf("found a body\n");
        const Body* b = (const Body*) node;
        if (b != p) {
            real d2 = mw_sqrv(mw_subv(Pos(b), Pos(p)));
            insertNeighbor(neighbors, dists, K, (Body*)b, d2);
        }
    }
    else if (isCell(node))
    {
	mw_printf("found a cell that isn't a body\n");
        const NBodyCell* c = (const NBodyCell*) node;

        // Compute distance from p to the cell bounding box
        real dx = mw_fmax(0.0, fabs(X(Pos(p)) - X(Pos(c))) - 0.5 * t->rsize);
        real dy = mw_fmax(0.0, fabs(Y(Pos(p)) - Y(Pos(c))) - 0.5 * t->rsize);
        real dz = mw_fmax(0.0, fabs(Z(Pos(p)) - Z(Pos(c))) - 0.5 * t->rsize);
        real boxDist2 = dx*dx + dy*dy + dz*dz;

        // Determine current worst distance
        real worstDist = dists[0];
        for (int i = 1; i < K; i++)
            if (dists[i] > worstDist) worstDist = dists[i];

        if (boxDist2 >= worstDist) return;
        ...
        for (int i = 0; i < NSUB; i++)
        {
            ...
            // Ensure type sanity
            if (!isBody(child) && !isCell(child)) {
                //mw_printf("Skipping unknown node type at %p\n", child);
                continue;
            }

            // Recurse safely
            searchNodeSafe(t, child, p, neighbors, dists, K);
        }
    }
    else
    {
        //mw_printf("Skipping node with unknown type at %p\n", node);
    }
}

static int nbNearestNeighbors(const NBodyTree* t, const Body* p, Body* out[4])
{
    //mw_printf("running nbNearestNeighbors\n");
    Body* neighbors[KN] = {NULL};
    real dists[KN];
    for (int i = 0; i < KN; i++) dists[i] = HUGE_VAL;

    /* recursive search using static function */
    searchNodeSafe(t, (const NBodyNode*) t->root, p, neighbors, dists, KN);

    /* collect valid neighbors into out[], sorted by distance */
    int found = 0;
    for (int i = 0; i < KN; i++)
        if (neighbors[i] != NULL) out[found++] = neighbors[i];
   ...
   /* for (int i = 0; i < found; i++)
    {
        ...
        if (best != i)
        {
            real td = dists[i]; dists[i] = dists[best]; dists[best] = td;
            Body* tb = out[i]; out[i] = out[best]; out[best] = tb;
        }
    }*/
...
/* Print nearest neighbors for particle p using the tree.
 * Must be called BEFORE hackCofM() replaces geometric centers with center-of-mass.
 */

int nbPrintNearestNeighbors(const NBodyTree* t, const Body* p, int particle_id, FILE* fdata, FILE* fvel,FILE* fdens)
{
    //const int K = 4;
    //mw_printf("running nbPrintNearestNeighbors\n");
    Body* neighbors[KN] = {NULL};
    int found = nbNearestNeighbors(t, p, neighbors);  // tree-based kNN

    mwvector pos0 = Pos(p);

    if (found > 0)
    {
        //mw_printf("Particle %d at (%.6f, %.6f, %.6f) - Nearest neighbors:\n",
          //        particle_id, pos0.x, pos0.y, pos0.z);

        for (int i = 0; i < found; i++)
        {
	    if (neighbors[i] == NULL) continue;
            mwvector npos = Pos(neighbors[i]);
            real dist = mw_sqrt(mw_sqrv(mw_subv(pos0, npos)));
            //mw_printf("  Neighbor %d: distance=%.6f, position=(%.6f, %.6f, %.6f), mass=%.6f\n",
            //          i+1, dist, npos.x, npos.y, npos.z, Mass(neighbors[i]));
        }
        //mw_printf("\n");
    }
    else
    {
        //mw_printf("Particle %d has no neighbors.\n", particle_id);
    }
    float  maxdistance = 0;

    for (int i = 0; i < (found-1); i++) {
	   //mw_printf("about to define npos\n");
           mwvector npos = Pos(neighbors[i]);
	   //mw_printf("about to define dist\n");
            real dist = mw_sqrt(mw_sqrv(mw_subv(pos0, npos)));
	   //mw_printf("about to compare\n");
        if (dist > maxdistance) {
            maxdistance = dist;
        }
    }
    //mw_printf("about to do math\n");
    real volume = 1.33333333 * 3.14159265358979323846 * mw_pow(maxdistance,3);
    //printf("maxdistance: %f\n",maxdistance);
    real total_mass = 32 * Mass(neighbors[0]);
    real local_density = total_mass/volume;
    //mw_printf("local_density: %f\n",local_density*222288.47);
    //mw_printf(".\n");
    //real local_velocity = mw_sqrt(mw_pow(neighbors[i]->vel.x,2)+mw_pow(neighbors[i]->vel.z,2)+mw_pow(neighbors[i]->vel.z,2));
    //mw_printf("local velocity: %f\n",local_velocity);
    real cross_section = 0.00004639155; // sq222288.47uare kiloparsecs per structure mass
    real total_probability = 1;
    FILE *fptr;
    //fptr = fopen("velocity.txt","a");
    for (int i=0; i<32; i++){
        real local_velocity = mw_sqrt(mw_pow(neighbors[i]->vel.x,2)+mw_pow(neighbors[i]->vel.z,2)+mw_pow(neighbors[i]->vel.z,2));
	//mw_printf("local velocity: %f\n",local_velocity);
	real probability = cross_section *  mw_sqrt(mw_pow(neighbors[i]->vel.x-p->vel.x,2)+mw_pow(neighbors[i]->vel.y-p->vel.y,2)+mw_pow(neighbors[i]->vel.z-p->vel.z,2)) * 0.004;
	total_probability = total_probability * (1-probability);
        //fptr = fopen("velocity.txt","a");

	//fprintf(fvel,"%f\n",local_velocity);

	//fclose(fptr);
	//mw_printf("probability: %f\n",probability);
    }
    //fclose(fptr);
    ...
    //mw_printf("random number: %f\n",r);
    //if (r<total_probability) {
	//mw_printf("scattered\n");
    //return 1;
    //} else {
	//mw_printf("didn't scatter\n");
	//return 0;
    //}

	//mw_printf("scattered\n");
    //mw_printf("total probability: %f\n",total_probability);
}
...
for (int i = 0; i < st->nbody; i++)
	...
	scatters = scatters + nbPrintNearestNeighbors(&st->tree, &st->bodytab[i], i, fdata, fvel, fdens);
    fclose(fvel);
    fclose(fdata);
    fclose(fdens);

    mw_printf("scatters: %d\n",scatters);
