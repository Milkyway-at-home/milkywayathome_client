...
int NearestNeighbors(NbodyState* st, Body* p){
	...
	/*for(int i=0; i<8; i++){
		if (isBody(Subp(q[i]))){ // is the current node a leaf?
			// add body to list of candidate nodes
		} else {
			// find the closest node and descend to the next level	
			q = (NBodyCell*) Subp(q)[i];
			currentlevel++;
		}
	}*/
	/*for(int i =0;i<8;i++){
		float present_size = total_size * (mw_pow(total_size,-currentlevel));
		real x_lower;
		real x_upper;
		real y_lower;
		real y_upper;
		real z_lower;
		real z_upper;
		if (isBody(Subp(q[i]))){ // is leaf?

		} else {
			//x_lower = body_pos[0] - psize * 0.5;
			x_lower = ;
		}
	}
	*/
