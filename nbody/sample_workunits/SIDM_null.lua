  ...
  local finalPosition, finalVelocity, LMCfinalPosition, LMCfinalVelocity
    ...
    if(ModelComponents == 2) then 
        ...
        firstModel = predefinedModels.mixeddwarf{
            ...
            comp1       = Dwarf.plummer{mass = mass_l, scaleLength = rscale_l}, -- Dwarf Options: plummer, nfw, general_hernquist, sidm
            comp2       = Dwarf.sidm{mass = mass_d, scaleLength = rscale_dm, r1 = rscale_dm, rc = rscale_dm}, -- Dwarf Options: plummer, nfw, general_hernquist, sidm
            ignore      = true
        }
...
assert(#arg >= 6, "Expects either 7, or 13 arguments, and optional manual body list")
...
rscale_dm        = (rscale_l * (1 - light_r_ratio)) / light_r_ratio -- Dark Matter Radius (kpc)
if (#arg >= 7) then
	sigma = round( tonumber(arg[7]), dec )
end
if (#arg >= 13) then
  orbit_parameter_l   = round( tonumber(arg[8]), dec )
  orbit_parameter_b   = round( tonumber(arg[9]), dec )
  orbit_parameter_r   = round( tonumber(arg[10]), dec )
  orbit_parameter_vx  = round( tonumber(arg[11]), dec )
  orbit_parameter_vy  = round( tonumber(arg[12]), dec )
  orbit_parameter_vz  = round( tonumber(arg[13]), dec )
  ...
  manual_body_file = arg[8] -- File with Individual Particles (.out file)
end
