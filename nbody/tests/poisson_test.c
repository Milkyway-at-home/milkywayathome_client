                                            ...
                                            real* rho0)
{
    ...
    *flattenY    = 1.0;
    *flattenZ    = randomValueReal(mw_sqrt_0(*flattenX * *flattenX / (*flattenX * *flattenX + 1.0)),(1.0/mw_pow_0(*flattenX,2.71828))+1);
...
static inline mwbool checkPoisson(const Potential* pot, mwvector* pos)//const pos
{
    ...
    real diff;
    real h_r = h * (sqr_0(X(pos))+sqr_0(Y(pos))+sqr_0(Z(pos)));
...
static inline mwbool testSphericalPotential(spherical_t t)
{
    ...
    for (j = 0; j < nPositions; j++)
    {
        ...
        {
            pos.x = randomValueReal(min_pos,max_pos);
            pos.y = randomValueReal(min_pos,max_pos);
            pos.z = randomValueReal(min_pos,max_pos);
...
static inline mwbool testBarPotential(disk_t t)
{
    ...
    for (j = 0; j < nPositions; j++)
    {
        ...
        while (poisson_check == 2)
        {
            ...
            pos.y = randomValueReal(-0.2,0.2);
            pos.z = randomValueReal(-0.2,0.02);
            while(mw_abs_0(X(&pos)) < 0.01){
                pos.x = randomValueReal(-4,4);
            }
            poisson_check = checkPoisson(&p,&pos);
        }
...
static inline mwbool testHaloPotential(halo_t t)
{
    ...
    for (j = 0; j < nPositions; j++)
    {
        ...
        while (poisson_check == 2)
        {
            ...
            mag = mw_sqrt_0(X(&pos)*X(&pos) + Y(&pos)*Y(&pos) + Z(&pos)*Z(&pos));
