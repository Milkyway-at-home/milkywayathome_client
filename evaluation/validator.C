...
int handle_wu(DB_VALIDATOR_ITEM_SET& validator, std::vector<VALIDATOR_ITEM>& items) {
	...
	if (wu.canonical_resultid) {
		...
		for (i=0; i<items.size(); i++) {
			...
			log_messages.printf(SCHED_MSG_LOG::MSG_NORMAL, "[WU#%d %s] handle_wu(): Analyzing result %d\n", wu.id, wu.name, result.id);
			if (result.id == wu.canonical_resultid) {
				canonical_result_index = i;
			}
		}
		if (canonical_result_index == -1) {
			log_messages.printf(SCHED_MSG_LOG::MSG_CRITICAL, "[WU#%d %s] Can't find canonical result %d\n", wu.id, wu.name, wu.canonical_resultid);
			return 0;
		}
		...
		for (i=0; i<items.size(); i++) {
			...
			switch (result.validate_state) {
				case VALIDATE_STATE_VALID:
					...
					if (max_credit_per_cpu_second && (result.granted_credit / result.cpu_time) > max_credit_per_cpu_second) {
						result.granted_credit = result.cpu_time * max_credit_per_cpu_second;
					}

					log_messages.printf(SCHED_MSG_LOG::MSG_NORMAL, "[RESULT#%d %s] pair_check() matched: setting result to valid; credit %f\n", result.id, result.name, result.granted_credit);
					retval = is_valid(result,wu);
					if (retval) {
						log_messages.printf(SCHED_MSG_LOG::MSG_NORMAL, "[RESULT#%d %s] Can't grant credit: %d\n", result.id, result.name, retval);
					}
					break;
				case VALIDATE_STATE_INVALID:
					update_result = true;
					log_messages.printf(SCHED_MSG_LOG::MSG_NORMAL, "[RESULT#%d %s] pair_check() didn't match: setting result to invalid\n", result.id, result.name);
					is_invalid(result);
			}
			if (update_result) {
		...
		// Try to get one
		log_messages.printf(SCHED_MSG_LOG::MSG_NORMAL, "[WU#%d %s] handle_wu(): No canonical result yet\n", wu.id, wu.name);
		++log_messages;
		...
		log_messages.printf(SCHED_MSG_LOG::MSG_DEBUG, "[WU#%d %s] Found %d successful results\n", wu.id, wu.name, (int)results.size());
		if (results.size() >= (unsigned int)wu.min_quorum) {
			log_messages.printf(SCHED_MSG_LOG::MSG_DEBUG, "[WU#%d %s] Enough for quorum, checking set.\n", wu.id, wu.name);

			retval = check_set(results, wu, canonicalid, credit, retry);
			if (retval) {
				log_messages.printf(SCHED_MSG_LOG::MSG_CRITICAL, "[WU#%d %s] check_set returned %d, exiting\n", wu.id, wu.name, retval);
				return retval;
			}
			if (retry) transition_time = DELAYED;
			if (credit_from_wu) {
			...
			for (i=0; i<results.size(); i++) {
				...
				switch (result.validate_state) {
					case VALIDATE_STATE_VALID:
						...
						retval = is_valid(result,wu);
						if (retval) {
							log_messages.printf(SCHED_MSG_LOG::MSG_DEBUG, "[RESULT#%d %s] is_valid() failed: %d\n", result.id, result.name, retval);
						}
			...
			if (canonicalid) {
				...
				log_messages.printf(SCHED_MSG_LOG::MSG_DEBUG, "[WU#%d %s] Found a canonical result: id=%d\n", wu.id, wu.name, canonicalid);
