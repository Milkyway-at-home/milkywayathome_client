...
void asynchronous_search(int number_arguments, char** arguments, int number_parameters, double *point, double *range, BOUNDS* bounds) {
	...
	int max_heap_size, time_template_filename;

	printf("Initializing search manager...\n");
	init_search_manager(number_arguments, arguments);
        for (i = 0; i < number_arguments; i++) {
			...
			position = manage_search(arguments[i]);
                        ...
                        break;
                } else if (!strcmp(arguments[i], "-time_template")) {
			time_template_filename = ++i;
		} else if (!strcmp(arguments[i], "-heap_size")) {
			max_heap_size = atoi(arguments[++i]);
		}
        }
        if (ms == NULL) return;
	...
	printf("Initializing simulation heap...\n");
	simulation_heap__init(max_heap_size, number_parameters, arguments[time_template_filename]);

	printf("Setting initial heap values...\n");
...
//		printf("inserting fitness: %lf\n", sp->fitness);
		simulation_heap__insert(sp);
//		printf("inserted.\n");
		simulation_heap__remove_min(sp);
//		printf("removed fitness: %lf\n", sp->fitness);
