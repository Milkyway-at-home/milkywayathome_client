...
--previousForwardTime  = 3.547421 --this is the forward time of a calibration run
--previousForwardTime = 3.53532 --standard run--
previousForwardTime = 3.559326
...
function get_timestep(totalTime)
    if (timestep_control) then
        t = (totalTime) / (Ntime_step)
    ...
    if (totalTime/t > 150000 or t ~= t) then
        TooManyTimesteps = 1
        t = totalTime/4.0
    end
...
function makeContext()
   ...
   return NBodyCtx.create{
      ...
      timeBack    = revOrbTime,
      timestep    = get_timestep(evolveTime),
      --timestep    = 0.00039161885317116,
  ...
  local finalPosition, finalVelocity, LMCfinalPosition, LMCfinalVelocity
    ...
    if(run_null_potential) then
        ...
        finalPosition, finalVelocity = reverseOrbit{
            potential = potential,
            position  = lbrToCartesian(ctx, Vector.create(orbit_parameter_l, orbit_parameter_b, orbit_parameter_r)),
            --position  = Vector.create(3.432568, -16.038259, 2.271027),--this works well
            --position = Vector.create(-10.757857, -14.799425, 15.111649),--single orbit based
            velocity  = Vector.create(orbit_parameter_vx, orbit_parameter_vy, orbit_parameter_vz),
            --velocity  = Vector.create(203.595269, 36.995849, -214.885884),--this works well
            --velocity = Vector.create(189.254975, -55.691665, -150.606089),--single orbit based
            tstop       = revOrbTime,
            --tstop       = 3.53532,
            dt        = ctx.timestep / 10.0,
            --dt          = 0.000039161885317116,
            forwardTime = previousForwardTime
            --forwardTime = revOrbTime
        }
        print("dt: ") 
        print(ctx.timestep / 10.0)
    end
end
    
    if(print_reverse_orbit) then
        local placeholderPos, placeholderVel = PrintReverseOrbit{
            potential = potential,
            --position  = lbrToCartesian(ctx, Vector.create(orbit_parameter_l, orbit_parameter_b, orbit_parameter_r)),
            position = Vector.create(-10.757857, -14.799425, 15.111649),
            --velocity  = Vector.create(orbit_parameter_vx, orbit_parameter_vy, orbit_parameter_vz),
            velocity = Vector.create(189.254975, -55.691665, -150.606089),
            tstop     = .14,
            tstopf    = .20,
            dt        = ctx.timestep / 10.0
        }
