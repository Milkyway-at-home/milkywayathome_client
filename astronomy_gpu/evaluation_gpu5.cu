...
void gpu__initialize(	int ap_wedge, int ap_convolve, int ap_number_streams, int ap_number_integrals, 
	...
	_integrals = (GPU_PRECISION**)malloc(number_integrals * sizeof(GPU_PRECISION*));
...
//		printf("Allocating %d bytes for integral data on GPU\n", (number_streams + 1) * integral_size[i] * sizeof(GPU_PRECISION));
		...
		cutilSafeCall( cudaMalloc((void**) &_integrals[i], integral_size[i] * sizeof(GPU_PRECISION)) );
...
void gpu__free_constants() {
	...
	for (i = 0; i < number_integrals; i++) {
		...
		cutilSafeCall( cudaFree(_integrals[i]) );
...
//		shared__r_point[threadIdx.x + blockDim.x] = constant__r_constants[pos];
//		shared__qw_r3_N[threadIdx.x + blockDim.x] = constant__r_constants[pos + 1];
//	}
...
void cpu__sum_integrals(int iteration, double *background_integral, double *stream_integrals) {
	...
	cutilSafeCall( cudaMemcpy(host__background_integrals[iteration], _integrals[iteration], integral_size[iteration] * sizeof(GPU_PRECISION), cudaMemcpyDeviceToHost) );
...
double gpu__likelihood(double *parameters) {
	...
	for (i = 0; i < number_integrals; i++) {
		...
		gpu__zero_integrals<2><<<dimGrid, nu_steps[i]>>>(_integrals[i], device__stream_integrals[i]);
